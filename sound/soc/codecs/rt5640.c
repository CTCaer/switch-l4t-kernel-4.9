...
static const struct reg_sequence init_list[] = {
	{RT5640_PR_BASE + 0x1d, 0x0347}, /* Set bit8. Should this be actually get set? */
	...
	{RT5640_PR_BASE + 0x23,	0x1804}, /* Set bit12. Unset produces speakers pop on power off */
};

/* Nintendo Switch (2017/2019) EQ presets */
static const struct reg_sequence eq_speakers_list_odin[] = {
	/* EQ LPF Bandwidth  (LPF:a1)  from 0.88 to -0.577 */
	{RT5640_PR_BASE + RT5640_EQ_BW_LOP,  0xed87},
	/* EQ LPF Gain       (LPF:H0)  from 0.06 to  0.000 */
	...
	/* EQ HPF2 Cutoff    (HPF2:a1) from 1.00 to  0.991 */
	{RT5640_PR_BASE + RT5640_EQ_FC_HIP2, 0x1fb4},
	/* EQ HPF2 Bandwidth (HPF2:a2) from 0.00 to  0.009 */
	{RT5640_PR_BASE + RT5640_EQ_BW_HIP2, 0x004b},
	/* EQ HPF2 Gain      (HPF2:H0) from 1.00 to  0.991 */
	{RT5640_PR_BASE + RT5640_EQ_GN_HIP2, 0x1fb4},

	/* EQ Pre  Volume reset val 1.0/0dB */
	{RT5640_PR_BASE + RT5640_EQ_PRE_VOL, 0x0800},
	/* EQ Post Volume reset val 1.0/0dB */
...
static const struct reg_sequence eq_speakers_list_vali[] = {
	/* EQ LPF Bandwidth  (LPF:a1)  from 0.88 to 0.268 */
...
static const struct reg_sequence eq_speakers_list_frig[] = {
	/* EQ LPF Bandwidth  (LPF:a1)  from 0.88 to -0.877 */
	...
	/* EQ HPF1 Cutoff    (HPF1:a1) from 1.00 to  0.954 */
	{RT5640_PR_BASE + RT5640_EQ_FC_HIP1, 0x1e88},
	/* EQ HPF1 Gain      (HPF1:H0) from 0.00 to  0.122 */
	{RT5640_PR_BASE + RT5640_EQ_GN_HIP1, 0x03ea},

	/* EQ HPF2 Cutoff    (HPF2:a1) from 1.00 to  0.996 */
	{RT5640_PR_BASE + RT5640_EQ_FC_HIP2, 0x1fe2},
	/* EQ HPF2 Bandwidth (HPF2:a2) from 0.00 to  0.004 */
	{RT5640_PR_BASE + RT5640_EQ_BW_HIP2, 0x001e},
	/* EQ HPF2 Gain      (HPF2:H0) from 1.00 to  0.996 */
	{RT5640_PR_BASE + RT5640_EQ_GN_HIP2, 0x1fe2},

	/* EQ Pre  Volume reset val 1.0/0dB */
	{RT5640_PR_BASE + RT5640_EQ_PRE_VOL, 0x0800},
	/* EQ Post Volume from 1.0 (0dB) to 0.804 (+x.xxdB) */
...
static const struct reg_sequence eq_microphone_list_nx[] = {
	...
	/* EQ HPF2 Cutoff    (HPF2:a1) from 1.00 to 0.981 */
	{RT5640_PR_BASE + RT5640_EQ_FC_HIP2, 0x1f68},
	/* EQ HPF2 Bandwidth (HPF2:a2) from 0.00 to 0.018 */
	{RT5640_PR_BASE + RT5640_EQ_BW_HIP2, 0x0094},
	/* EQ HPF2 Gain      (HPF2:H0) from 1.00 to 0.982 */
			   ...
			   struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		/* Set Speakers EQ */
		dev_dbg(codec->dev, "Enabling Speakers EQ\n");
		...
		if (ret != 0)
			dev_warn(codec->dev, "Failed to apply spk EQ: %d\n", ret);
		break;

	case SND_SOC_DAPM_PRE_PMD:
		/* Disable Speakers EQ. */
		dev_dbg(codec->dev, "Disabling EQ\n");
		ret = regmap_multi_reg_write(rt5640->regmap, eq_normal_list_nx,
						ARRAY_SIZE(eq_normal_list_nx));
		if (ret != 0)
			dev_warn(codec->dev, "Failed to disable EQ: %d\n", ret);
		break;
...
static bool rt5640_micbias1_ovcd_irq_enabled(struct snd_soc_codec *codec)
{
	int val = snd_soc_read(codec, RT5640_IRQ_CTRL2);

	return (val & RT5640_IRQ_MB1_OC_MASK);
}
...
jack_removed:
		...
		if (rt5640->jack->status & SND_JACK_HEADPHONE) {
			...
			if (rt5640->ext_mic)
				snd_soc_update_bits(codec, RT5640_IRQ_CTRL2,
					RT5640_MB1_OC_STKY_MASK, 0);
			rt5640_disable_micbias1_for_ovcd(codec);

			snd_soc_jack_report(rt5640->jack,
				0, SND_JACK_HEADSET | SND_JACK_BTN_0);
			dev_dbg(codec->dev, "jack unplugged\n");
		} else if (rt5640->jack_init_state) {
			snd_soc_jack_report(rt5640->jack,
				0, SND_JACK_HEADSET | SND_JACK_BTN_0);
			rt5640->jack_init_state = false;
			dev_dbg(codec->dev, "jack init state unplugged\n");
		}
		...
		if (status & SND_JACK_MICROPHONE) {
			/* Enable sticky OVCD for button identification */
			snd_soc_update_bits(codec, RT5640_IRQ_CTRL2,
	...
	} else if (rt5640_micbias1_ovcd_irq_enabled(codec)) {
		if (rt5640->jack->status & SND_JACK_MICROPHONE) {
			/* Check that jack is still connected before checking ovcd */
			msleep(JACK_SETTLE_TIME);
			if (!rt5640_jack_inserted(codec))
				goto jack_removed;

			status = rt5640_micbias1_ovcd(codec) ? SND_JACK_BTN_0 : 0;
			dev_dbg(codec->dev, "button status %#02x\n", status);
			snd_soc_jack_report(rt5640->jack, status, SND_JACK_BTN_0);
			rt5640_clear_micbias1_ovcd(codec);
		}
	}
}
				      ...
				      struct snd_soc_jack *jack)
{
	...
	if (rt5640->sel_jd_source >= 0)
		snd_soc_update_bits(codec, RT5640_JD_CTRL,
			RT5640_JD_MASK, rt5640->sel_jd_source << RT5640_JD_SFT);
	else
		snd_soc_write(codec, RT5640_JD_CTRL,
			RT5640_JD_SRC_JD2_IN4N << RT5640_JD_SFT);

	/* Selecting GPIO01 as an interrupt */
	snd_soc_update_bits(codec, RT5640_GPIO_CTRL1,
		RT5640_GP1_PIN_MASK | RT5640_DP_SIG_MASK,
		RT5640_GP1_PIN_IRQ | RT5640_DP_SIG_AP);
	...
	if (!rt5640->jd1_only)
		...
		snd_soc_write(codec, RT5640_GEN_CTRL2, 0x0000);
	 ...
	 * External mic detect needs special handling.
	 */
	snd_soc_update_bits(codec, RT5640_IRQ_CTRL2,
		RT5640_MB1_OC_STKY_MASK, !rt5640->ext_mic ? RT5640_MB1_OC_STKY_EN : 0);
	 ...
	 */
	if (rt5640->jd_inverted)
		snd_soc_write(codec, RT5640_IRQ_CTRL1, RT5640_IRQ_JD_NOR);
	else
...
static int rt5640_parse_dt(const struct device *dev, struct rt5640_priv *rt5640,
		    struct device_node *np)
{
	...
	rt5640->ext_mic = of_property_read_bool(np,
				"realtek,external-microphone");

	rt5640->jack_gpio = of_get_named_gpio(np,
					"realtek,jack-detect-gpios", 0);

	if (of_property_read_s32(np, "realtek,sel-jd-source",
		&rt5640->sel_jd_source)) {
	...
	if (of_property_read_u32(np,
		...
		case 2000:
			...
			break;
		default:
			dev_warn(dev, "invalid ovc threshold");
		}
	}
	...
	if (of_property_read_u32(np,
		...
		if (val <= RT5640_OVCD_SF_1P5)
			...
			dev_warn(dev, "invalid ovc scale");
	}
	/*
		    ...
		    const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		ret = rt5640_parse_dt(&i2c->dev, rt5640, i2c->dev.of_node);
		if (ret)
			return ret;
	} else
		rt5640->pdata.ldo1_en = -EINVAL;
