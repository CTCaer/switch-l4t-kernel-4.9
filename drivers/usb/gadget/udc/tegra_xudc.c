...
static struct tegra_xudc_soc_data tegra210_xudc_soc_data = {
	...
	.pgid_dev = TEGRA210_POWER_DOMAIN_XUSBB,
	.u1_enable = false,
	.u2_enable = true,
	.lpm_enable = false,
...
static struct tegra_xudc_soc_data tegra186_xudc_soc_data = {
	...
	.pgid_dev = TEGRA186_POWER_DOMAIN_XUSBB,
	.u1_enable = true,
	.u2_enable = true,
	.lpm_enable = false,
	.invalid_seq_num = false,
...
static struct tegra_xudc_soc_data tegra194_xudc_soc_data = {
	...
	.pgid_dev = TEGRA194_POWER_DOMAIN_XUSBB,
	.u1_enable = true,
	.u2_enable = true,
	.lpm_enable = true,
	.invalid_seq_num = false,
...
static int tegra_xudc_probe(struct platform_device *pdev)
{
	...
	if (xudc->soc->pgid_ss < 0) {
		err = -EINVAL;
		goto disable_clk;
	}
	err = tegra_unpowergate_partition(xudc->soc->pgid_ss);
	if (err < 0) {
		dev_err(xudc->dev, "failed to unpowergate XUSBA partition\n");
		goto disable_clk;
	}

	if (xudc->soc->pgid_dev < 0) {
		err = -EINVAL;
		goto powergate_xusba;
	}
	err = tegra_unpowergate_partition(xudc->soc->pgid_dev);
	if (err < 0) {
...
powergate_xusbb:
	tegra_powergate_partition(xudc->soc->pgid_dev);
powergate_xusba:
	tegra_powergate_partition(xudc->soc->pgid_ss);
disable_clk:
