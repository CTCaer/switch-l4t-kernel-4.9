...
static int joycon_get_ctlr_info(struct joycon_ctlr *ctlr)
{
	...
	switch (report->subcmd_reply.data[2]) {
	case 1:
...
static int joycon_request_calibration(struct joycon_ctlr *ctlr)
{
	...
	dev_dbg(&ctlr->sdev->dev, "requesting cal data\n");
	...
	/* stick calibration parsing */
	cal_x = &ctlr->stick_cal_x;
	cal_y = &ctlr->stick_cal_y;
	...
	} else if (ctlr->ctlr_type == JOYCON_TYPE_RIGHT) {
		raw_cal += 9;
...
static int joycon_request_imu_calibration(struct joycon_ctlr *ctlr)
{
	...
	dev_dbg(&ctlr->sdev->dev, "requesting IMU cal data\n");
	ret = joycon_send_subcmd(ctlr, req, 5, HZ);
	...
	dev_dbg(&ctlr->sdev->dev, "IMU calibration:\n"
...
static int joycon_input_create(struct joycon_ctlr *ctlr)
{
	...
	case JOYCON_TYPE_LEFT:
		...
		imu_name = "Nintendo Switch Left Joy-Con Serial IMU";
		break;
	case JOYCON_TYPE_RIGHT:
		name = "Nintendo Switch Right Joy-Con Serial";
		imu_name = "Nintendo Switch Right Joy-Con Serial IMU";
		break;
	...
	ctlr->imu_input->id.product = type == JOYCON_TYPE_LEFT ? 0x2006 :
								 0x2007;
...
static int joycon_post_handshake(struct joycon_ctlr *ctlr)
{
	...
	if (ret) {
		 ...
		 * inaccurate. Provide a warning, and continue on.
		 */
		dev_warn(dev, "Unable to read IMU calibration data\n");
	}
	...
	mutex_unlock(&ctlr->output_mutex);
	if (ret) {
		dev_err(dev, "Failed to enable the IMU; ret=%d\n", ret);
		goto error;
	}
	...
	if (unlikely(mutex_is_locked(&ctlr->output_mutex))) {
		...
		case JOYCON_MSG_TYPE_SUBCMD:
			if (packet->command == JC_CMD_EXTRET &&
				...
				if (r->subcmd_reply.id != ctlr->subcmd_ack_match)
					break;
