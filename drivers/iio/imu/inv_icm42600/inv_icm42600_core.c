...
static const struct inv_icm42600_hw inv_icm42600_hw[INV_CHIP_NB] = {
	...
	[INV_CHIP_ICM40607] = {
		.whoami = INV_ICM42600_WHOAMI_ICM40607,
		.name = "icm40607",
		.conf = &inv_icm42600_default_conf,
	},
	[INV_CHIP_ICM42602] = {
				 ...
				 int irq_type, bool open_drain, bool irq_int2)
{
	...
	if (!irq_int2) {
		...
		case IRQF_TRIGGER_RISING:
		case IRQF_TRIGGER_HIGH:
			val = INV_ICM42600_INT_CONFIG_INT2_ACTIVE_HIGH;
			break;
		default:
			val = INV_ICM42600_INT_CONFIG_INT2_ACTIVE_LOW;
			break;
		}
		...
		case IRQF_TRIGGER_LOW:
		case IRQF_TRIGGER_HIGH:
			val |= INV_ICM42600_INT_CONFIG_INT2_LATCHED;
			break;
		default:
			break;
		}
		...
		if (!open_drain)
			val |= INV_ICM42600_INT_CONFIG_INT2_PUSH_PULL;
	}
			    ...
			    inv_icm42600_bus_setup bus_setup)
{
	...
	ret = inv_icm42600_irq_init(st, irq, irq_type, open_drain, irq_int2);
	if (ret)
		return ret;
